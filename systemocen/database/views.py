...
def studentpage(request, page_id):
    if request.user.is_authenticated():
        try:
            ...
            subjectsAct = (subjectAct for subjectAct in
                           SubjectsStudents.objects.filter(student_id=st).filter(term_number=st.term_number))
            ...
            return render(request, 'database/studentpage.html',
                          {'student': st, 'subjectsStudents': subjectsStudents, 'messages': messages,
                           'all_messages': allMessages,
                           'page_id': page_id, 'subsubjectsStudents': subsubjectsStudents, 'subjectsAct': subjectsAct,
                           'subjectsArch': subjectsArch, 'survey': survey})
        except Student.DoesNotExist:
...
def teacherviewsubjectfinalgrades(request, sbjct_id):
    if request.user.is_authenticated():
        try:
            ...
            subjectsStudents = (subjectStudent for subjectStudent in
                                SubjectsStudents.objects.filter(subject_id=sbjct_id))
            return render(request, 'database/teacherfinalgrades.html',
                          {'teacher': te, 'subject': subject, 'subjectsStudents': subjectsStudents})
        except Teacher.DoesNotExist:
    ...
    if request.user.is_authenticated():
        try:
            ...
            if (request.POST.get('create_message', False) and request.POST.get('student_id', False)):
                ...
                message = Message(student_id=messageTo, teacher_id=te, contents=request.POST.get('message'),
                                  is_read=False, date=timezone.now())
                message.save()
...
def teachersubject(request, subject_id):
    if request.user.is_authenticated():
        try:
            ...
            return render(request, 'database/teachersubject.html',
                          {'teacher': te, 'subject': subject, 'students': students})
        except Teacher.DoesNotExist:
            return HttpResponse("Nie ma nauczyciela")
        except Subject.DoesNotExist:
            return HttpResponse("Brak przedmiotu")
...
def teachersubsubject(request, subsubject_id):
    if request.user.is_authenticated():
        try:
            ...
            students = (subsubjectStudent.student_id for subsubjectStudent in
                        SubsubjectsStudents.objects.filter(sub_subject_id=subsubject).all())
            return render(request, 'database/teachersubsubject.html',
                          {'teacher': te, 'subsubject': subsubject, 'students': students})
        except Teacher.DoesNotExist:
            return HttpResponse("Nie ma nauczyciela")
        except Subject.DoesNotExist:
            return HttpResponse("Brak przedmiotu")
    ...
    if request.user.is_authenticated():
        try:
            ...
            return render(request, 'database/teacherstudent.html',
                          {'teacher': te, 'subject': subject, 'student': st, 'subGrades': subGrades,
                           'subjectsStudents': subjectsStudents, 'subsubjectsStudents': subsubjectsStudents})
        except Student.DoesNotExist:
...
def subteacherstudent(request, subsubject_id, student_id):
    if request.user.is_authenticated():
        try:
            ...
            if subsubject.teacher_id != te:
                return HttpResponse("Nie prowadzisz tego przedmiotu")
            subsubjectStudent = SubsubjectsStudents.objects.filter(student_id=student_id).get(
                sub_subject_id=subsubject_id)
            st = Student.objects.get(pk=student_id)
            ...
            return render(request, 'database/subteacherstudent.html',
                          {'teacher': te, 'subsubject': subsubject, 'student': st, 'subgrades': subgrades,
    ...
    if request.user.is_authenticated():
        if request.user.is_superuser:
            try:
                ...
                return render(request, 'database/addstudentstosubject.html',
                          {'student': st, 'subject': subjects})
            except Subject.DoesNotExist:
                return HttpResponse("Brak przedmiotu")
        else:
    ...
    if request.user.is_authenticated():
        try:
            ...
            if SubjectsStudents.objects.filter(student_id=st.pk).filter(subject_id=subsubject.subject_id.pk):
                subgrade = Subgrade(student_id=st, date=timezone.now(), sub_subject_id=subsubject, value=grade_value,
                                    teacher_id=te)
                subgrade.save()
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        except Student.DoesNotExist:
...
def studentsubject(request, subject_id):
    if request.user.is_authenticated():
        try:
            ...
            subGrades = (subGrade for subGrade in Subgrade.objects.all().filter(student_id=st).filter(
                sub_subject_id__subject_id=subject).order_by('-date'))
            return render(request, 'database/studentsubject.html',
                          {'student': st, 'subject': subject, 'subGrades': subGrades})
        except Teacher.DoesNotExist:
            return HttpResponse("Nie ma studenta")
        except Subject.DoesNotExist:
            return HttpResponse("Brak przedmiotu")
...
def studentsubsubject(request, subsubject_id):
    if request.user.is_authenticated():
        try:
            ...
            return render(request, 'database/studentsubject.html',
                          {'student': st, 'subsubject': subsubject, 'grades': grades})
        except Teacher.DoesNotExist:
            return HttpResponse("Nie ma studenta")
        except Subject.DoesNotExist:
            return HttpResponse("Brak przedmiotu")
...
def studentaddsurvey(request):
    if request.user.is_authenticated():
        try:
            ankieta = request.POST['ankieta']
            uwagi = request.POST['uwagi']
            subject_id = request.POST['subject_id']
            teacher_grade = request.POST['teacher_grade']
            #te = Teacher.objects.get(pk=request.POST['teacher_id'])
            st = Student.objects.get(user=request.user)
            #subject = Subject.objects.get(student_id=st)
            #subject = Survey.objects.get(pk=subject_id)
            #subjectStudent = SubjectsStudents.objects.get(subject_id=subject, student_id=st)
            
            tgrade = Survey.objects.get(student_id=st)
            tgrade.teacher_grade = ankieta;
            tgrade.comments = uwagi;

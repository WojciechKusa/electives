...
<?php
		...
		echo '<thead><tr><th>ID</th><th>Name</th><th>ECTS</th><th>Hours</th><th>Min students</th>
                    <th>Max students</th><th>Actual number</th><th>Coordinator</th><th>Semester</th><th></th></tr></thead>
		<tbody>';
		...
		while ($j != count($courses)) {
				  ...
				  echo '<td><a href="index.php?controller=store&amp;action=viewCourse&amp;id='.$courses[$j]['idcourse'].'">'.$courses[$j][0].'</a></td>';
				  echo '<td>'.$courses[$j][2].' pkt</td>';
                                  ...
                                  if($courses[$j][11] < $courses[$j][6]) {
                                    echo '<td><button type="button" class="btn btn-success" onClick="location.href=\'index.php?controller=student&amp;action=signIn&amp;id='.$courses[$j]['idcourse'].'\'">Sign in</button></td>';
                                  } else {
                                      echo '<td><button class="btn btn-danger">Full</button></td>';
                                  }
				  echo '</tr>';
